...
                
                // --- START OF NEW LOGIC ---
                // Check if 'user_activationstatus' key is available and true
                const isNewUserActivation = !!(data.config && data.config.user_activationstatus === true);
                console.log('[Botbuster Debug] isNewUserActivation:', isNewUserActivation);
                // --- END OF NEW LOGIC ---

                // Construct the final iframe URL with configuration data
                iframe.src = `https://dev.botbuster.io/session_id=QC-12345&skin_type=${data.captcha_uid}&email=${finalEmail}&mfa=${hasEmailOption(data?.config?.mfa)}&website_url=${normalizeToWWW(loadedWebsiteUrl)}&new_user_activation=${isNewUserActivation}`;
                
